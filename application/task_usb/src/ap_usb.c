...
INT32S Read_10(str_USB_Lun_Info* lUSB_LUN_Read)
{
    ...
    signed int ret;
    unsigned int SCSI_LBA,SCSI_Transfer_Length;
    unsigned int* USB_RW_Buffer_PTR_A;
    unsigned int* USB_RW_Buffer_PTR_B;
    unsigned int* USB_RW_Buffer_PTR_Temp;
	...
	SCSI_LBA=GetLBA();
	SCSI_Transfer_Length=GetTransLeng();
	USB_RW_Buffer_PTR_A=(unsigned int*) BUF ;//??
	USB_RW_Buffer_PTR_B=USB_RW_Buffer_PTR_A+ 128;

	if ((SCSI_LBA + SCSI_Transfer_Length)>lUSB_LUN_Read->ulSecSize)	//huck
	{
	 DBG_PRINT("A2\r\n");
			Sense_Code = 0x1B;
			return -11;
	...
	{
            //ret = DrvSDCReadMulData(USB_RW_Buffer_PTR_A, 1);
           // NAND_ReadSector(SCSI_LBA+i,1,USB_RW_Buffer_PTR_A);
            ret =FileSysDrv[lUSB_LUN_Read->unLunType]->Drv_ReadSector(SCSI_LBA+i,1,(unsigned int)USB_RW_Buffer_PTR_A);
			if (stage)
			{
				ret = Send_To_USB_DMA_USB(USB_RW_Buffer_PTR_A,1);
				//Check if timeout
				if(ret != 0)
				{
					 DBG_PRINT("A3\r\n");
			...
			ret = Send_To_USB_DMA_USB(USB_RW_Buffer_PTR_A,0);
			if (ret != 0x00)
			{
				 DBG_PRINT("A4\r\n");
				ret = 0xffff;
...
INT32S Write_10(str_USB_Lun_Info* lUSB_LUN_Write)
{
   unsigned int j,ret,ret1,stage;
	...
	DBG_PRINT("W10 %x %x\r\n",SCSI_LBA,SCSI_Transfer_Length);
	if ((SCSI_LBA + SCSI_Transfer_Length)>lUSB_LUN_Write->ulSecSize)	//huck
	{
			Sense_Code = 0x1B; //LOGICAL BLOCK ADDRESS OUT OF RANGE
			return 1;
	}
	for(j=0; j<SCSI_Transfer_Length; j++)
	{
			...
			ret=Receive_From_USB_DMA_USB(USB_RW_Buffer_PTR_B,0);
			ret=Receive_From_USB_DMA_USB(USB_RW_Buffer_PTR_B,1);
			if(ret != 0x00)
			{
				break;
			}
			USB_RW_Buffer_PTR_Temp = USB_RW_Buffer_PTR_B;
            //ret1 = DrvSDCWriteMulData(USB_RW_Buffer_PTR_B, 1);
           // ret1 =NAND_WriteSector(SCSI_LBA+j,1,USB_RW_Buffer_PTR_B);
           ret1 = FileSysDrv[lUSB_LUN_Write->unLunType]->Drv_WriteSector(SCSI_LBA+j,1,(INT32U)USB_RW_Buffer_PTR_B);
			if(ret1 != 0x00)
			{
				ret = ret1;
				break;
