#ifndef __JPEG_HEADER_H__
#define __JPEG_HEADER_H__

INT8U jpeg_picture_header[624] = {	//这个数组是 for 照相;
/* 如何修改拍照分辨率?  2011-11-25 10:03  台湾卢工指导:
修改分辨率应修改以下 3 处, 可以修改任意分辨率:
<1> 修改 customer.h 中以下两个标号的定义:
#define PIC_WIDTH			1600
#define PIC_HEIGH			1200
<2> 修改本数组的第 1 行的 4 个字节: (不用修改)
<3> 修改本数组的第 12 行的 2 个字节:(不用修改)
*/
	// Start of image at 0x0. Start of frame at 0x2 
        0xFF, 0xD8, 0xFF, 0xC0, 0x00, 0x11, 0x08, (PIC_HEIGH>>8), (PIC_HEIGH&0xFF), (PIC_WIDTH>>8), (PIC_WIDTH&0xFF), 0x03, 0x01, 0x21, 0x00, 0x02, //PIC_WIDTH x PIC_HEIGH
//        0xFF, 0xD8, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x04, 0xB0, 0x06, 0x40, 0x03, 0x01, 0x21, 0x00, 0x02, //1600x1200
//        0xFF, 0xD8, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x09, 0x90, 0x0C, 0xC0, 0x03, 0x01, 0x21, 0x00, 0x02,		//3264x2448
        // Comment starts at 0x15 
        0x11, 0x01, 0x03, 0x11, 0x01, 0xFF, 0xFE, 0x00, 0x0B, 0x47, 0x50, 0x45, 0x6E, 0x63, 0x6F, 0x64, 
        // Luminance quantization table at 0x22, entries(64) at 0x27 
        0x65, 0x72, 0xFF, 0xDB, 0x00, 0x43, 0x00, 0x10, 0x0B, 0x0C, 0x0E, 0x0C, 0x0A, 0x10, 0x0E, 0x0D, 
        0x0E, 0x12, 0x11, 0x10, 0x13, 0x18, 0x28, 0x1A, 0x18, 0x16, 0x16, 0x18, 0x31, 0x23, 0x25, 0x1D, 
        0x28, 0x3A, 0x33, 0x3D, 0x3C, 0x39, 0x33, 0x38, 0x37, 0x40, 0x48, 0x5C, 0x4E, 0x40, 0x44, 0x57, 
        0x45, 0x37, 0x38, 0x50, 0x6D, 0x51, 0x57, 0x5F, 0x62, 0x67, 0x68, 0x67, 0x3E, 0x4D, 0x71, 0x79, 
        // Chrominance quantization table at 0x67, entries(64) at 0x6C 
        0x70, 0x64, 0x78, 0x5C, 0x65, 0x67, 0x63, 0xFF, 0xDB, 0x00, 0x43, 0x01, 0x11, 0x12, 0x12, 0x18, 
        0x15, 0x18, 0x2F, 0x1A, 0x1A, 0x2F, 0x63, 0x42, 0x38, 0x42, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 
        0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 
        0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 
        // Restart interval at 0xAC 
        0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0xFF, 0xDD, 0x00, 0x04, 
        // Huffman table Class=0(DC), ID=0 at 0xB2 
        ((PIC_WIDTH/4)>>8), ((PIC_WIDTH/4)&0xFF), 0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 	//PIC_WIDTH x PIC_HEIGH
//        0x01, 0x90, 0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 	//1600x1200
//        0x03, 0x30, 0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,		//3264x2448
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 
        // Huffman table Class=1(AC), ID=0 at , 
        // Huffman table Class=0(DC), ID=1 at 0x18A 
        0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFF, 0xC4, 0x00, 0x1F, 0x01, 0x00, 
        0x03, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
        // Huffman table Class=1(AC), ID=1 at 0x1AB 
        , 
        // Start of scan at 0x262 
        0xF9, 0xFA, 0xFF, 0xDA, 0x00, 0x0C, 0x03, 0x01, 0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3F, 0x00
};

INT8U jpeg_avi_header[624] = {	//这个数组是 for 录像;
	0xFF, 0xD8, 0xFF, 0xFE, 0x00, 0x0B, 0x47, 0x50, 0x45, 0x6E, 0x63, 0x6F, 0x64, 0x65, 0x72, 0xFF, 
	// Quality 50
	, 
    // Quality 50
	
};

// mpeg4 header
INT8U mpeg4_header_vga[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x04,0x06,0xC5,0xD4,0xC3,0x07,0xD3,0x14,0x04,0x3C,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};

INT8U mpeg4_header_qvga[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x04,0x06,0xC5,0xD4,0xC3,0x07,0xD3,0x0A,0x04,0x1E,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};

INT8U mpeg4_header_qqvga[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x04,0x06,0xC5,0xD4,0xC3,0x07,0xD3,0x05,0x04,0x0F,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};

INT8U mpeg4_header_cif[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x30,0x2E,0xC5,0xD4,0xC3,0x07,0xD3,0x0B,0x04,0x24,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};

INT8U mpeg4_header_qcif[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x30,0x2E,0xC5,0xD4,0xC3,0x07,0xD3,0x05,0x84,0x12,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};

INT8U mpeg4_header_sqcif[44] = {
	0x00,0x00,0x01,0xB0,0x03,0x00,0x00,0x01,0xB5,0x09,0x00,0x00,0x01,0x00,0x00,0x00,
	0x01,0x20,0x00,0xBC,0x04,0x06,0xC5,0xD4,0xC3,0x07,0xD3,0x04,0x04,0x0C,0x14,0x63,
	0x00,0x00,0x01,0xB2,0x58,0x76,0x69,0x44,0x30,0x30,0x34,0x36
};


#endif //__JPEG_HEADER_H__