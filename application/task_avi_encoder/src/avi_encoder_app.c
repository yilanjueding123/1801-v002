...
#include "jh_define.h"	//xiangyong 2012-4-9 16:50
...
static void avi_encode_sync_timer_isr(void)
{
	...
	if((pAviEncPara->tv - pAviEncPara->Tv) > 0)
	{
		...
		{
			OSQPost(AVIEncodeApQ, (void*)MSG_AVI_ONE_FRAME_ENCODE);
...
INT32S avi_encode_set_file_handle_and_caculate_free_size(AviEncPacker_t *pAviEncPacker, INT16S FileHandle)
{
    ...
    {//xiangyong 2012-4-9 16:50
    	...
    	gp_strcpy((INT8S*)pAviEncPacker->index_path, (INT8S*)"C:\\DCIM\\100MEDIA\\");
		#else
    	gp_strcpy((INT8S*)pAviEncPacker->index_path, (INT8S*)"C:\\VIDEO\\");
		#endif
    }
    else if(disk_no == 3)
...
/*	INT32U len, buf;
	...
	*/
	file_cat(pAviEncPacker->file_handle,pAviEncPacker->index_handle);
...
//	mm_dump();
	...
	//buffer_size = pAviEncVidPara->encode_width * pAviEncVidPara->encode_height >> 1;
	//2011-11-24 19:07 xiangyong: buffer_size 不应小于 98KB.
#if VIDEO_ENCODE_USE_MODE ==VIDEO_ENCODE_WITH_FIFO_IRQ
	buffer_size = 200*1024;	//2011-9-23 11:13
#else
	//buffer_size = 94*1024;	//2011-9-23 11:13
	buffer_size = (94+32)*1024;	//2011-9-23 11:13
#endif
...
//		mm_dump();
		if(!pAviEncVidPara->video_encode_addr[i]) RETURN(STATUS_FAIL);
...
static INT32S AviPacker_mem_alloc(AviEncPacker_t *pAviEncPacker)
{
	...
	if(!pAviEncPacker->file_write_buffer)
		pAviEncPacker->file_write_buffer = (INT32U *)gp_malloc_align(FileWriteBuffer_Size, 32);
	
	if(!pAviEncPacker->file_write_buffer) RETURN(STATUS_FAIL);
...
INT32S avi_encode_memory_alloc(void)
{
	...
	if(AviPacker_mem_alloc(pAviEncPacker0) < 0) {
		...
		write_buff_addr = (INT32U) pAviEncPacker0->file_write_buffer;
	}
...
void vid_enc_csi_fifo_end(INT8U flag, INT8U type)
{
	...
	if (type) {
		OSQPost(vid_enc_task_q, (void *) (ready_frame | 0x80000000));
	} else {
		OSQPost(vid_enc_task_q, (void *) ready_frame);
	}
}
...
INT32S avi_audio_memory_allocate(INT32U	cblen)
{
	...
	{
		pAviEncAudPara->pcm_input_addr[i] = (INT32U) gp_malloc_align(cblen, 16);
		if(!pAviEncAudPara->pcm_input_addr[i]) RETURN(STATUS_FAIL);
	...
	if(!pAviEncAudPara->pack_buffer_addr) RETURN(STATUS_FAIL);
