...
{//拍照:
	...
	OSQPost(AVIEncodeApQ, (void *) MSG_AVI_CAPTURE_PICTURE);
	{
		...
		{//delay 10s:
			OSTimeDly(1);
			if (g_capture_complete_flag) break;
		}
	}
#endif
	return nRet;
...
{//录像共有 4+1 个task: 写卡task(即 Packer Task)出错时会调用本函数:
	DEBUG_MSG(DBG_PRINT("AviPacker-ErrID = 0x%x!!!\r\n", ErrCode));	
	avi_encode_video_timer_stop();	//停止 Packer Timer;
	if (ErrCode == AVIPACKER_RESULT_FILE_WRITE_ERR || ErrCode == AVIPACKER_RESULT_IDX_FILE_WRITE_ERR) {
		...
		//上一行消息发送之后: 录像task会停止当前录像文件, 然后再返回到拍照模式;
		g_avi_packer_error_flag = 1;
	}
...
void avi_encode_state_task_entry(void *para)
{
    ...
    {      
        ...
        case MSG_AVI_START_SENSOR:	//sensor
          	jpeg_picture_header[7] = PIC_HEIGH >> 8;
			jpeg_picture_header[8] = PIC_HEIGH & 0xFF;
			jpeg_picture_header[9] = PIC_WIDTH >> 8;
			jpeg_picture_header[10] = PIC_WIDTH & 0xFF;
			jpeg_picture_header[176] = (PIC_WIDTH>>2) >> 8;
			jpeg_picture_header[177] = (PIC_WIDTH>>2) & 0xFF;
        ...
        case MSG_AVI_CAPTURE_PICTURE:
        	{
				...
				scale_up_and_encode(pAviEncVidPara->csi_frame_addr[0], write_buff_addr,
										 PIC_WIDTH, PIC_HEIGH, pAviEncVidPara->video_encode_addr[0] + 624);
#else
				scale_up_and_encode(write_buff_addr, write_buff_addr + 691200,
										 PIC_WIDTH, PIC_HEIGH, pAviEncVidPara->video_encode_addr[0] + 624);
#endif				
				...
				//上一行消息发送之后, AQP会执行: DBG_PRINT("Capture OK\r\n");
				g_capture_complete_flag = 1;
	            OSMboxPost(avi_encode_ack_m, (void*)C_ACK_SUCCESS);
	       		break;
AVI_CAPTURE_PICTURE_FAIL: 
	        	//flag = -1;
	        	//msgQSend(ApQ, MSG_STORAGE_SERVICE_PIC_DONE, &flag, sizeof(INT8S), MSG_PRI_NORMAL);
	        	//上一行消息发送之后, AQP会执行: DBG_PRINT("Capture Fail\r\n");
	        	g_capture_complete_flag = 2;
	        	OSMboxPost(avi_encode_ack_m, (void*)C_ACK_FAIL);
	        	break;
			}
       	case MSG_AVI_STORAGE_FULL:
       		{
				...
				//上一行消息发送之后: PeripheralTaskQ 会将LED闪烁N次后关机;
       		}
       	...
       	case MSG_AVI_ONE_FRAME_ENCODE:
			...
			if(pAviEncPara->ready_frame == 0) {
				goto EncodeNullFrame;	//goto EndofEncodeOneFrame;
			}
			
			video_stream = pAviEncPara->ready_size + 8 + 2*16;
			ret = avi_encode_disk_size_is_enough(video_stream);
			if (ret == 0) {
