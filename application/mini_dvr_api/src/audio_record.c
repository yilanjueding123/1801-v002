...
#if FileWriteBuffer_Size > (C_AUD_PCM_BUFFER_NO*C_WAVE_RECORD_BUFFER_SIZE*2 + C_BS_BUFFER_SIZE) 
...
{//此函数在录像时不会被调用;
	...
	if (pAudio_Encode_Para->FileLenth > AUD_FILE_MAX_RECORD_SIZE)
	{
		...
		//上一行消息发送之后: 录音task会停止当前录音文件, 然后再新建一个录音文件后再录音;
	}
	if (pAudio_Encode_Para->FileLenth > pAudio_Encode_Para->disk_free_size) {
