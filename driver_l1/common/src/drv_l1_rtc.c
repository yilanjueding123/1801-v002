...
static void rtc_isr()
{
	...
	//gpx_rtc_write(0x09,0xff);	//2011-1-7 21:26
...
void rtc_init(void)
{
	...
	gpx_rtc_write(0x0a,0);		//2011-1-7 21:26
...
{	
	...
	//R_RTC_MIN = (INT32U)rtc_time->rtc_min;
	R_RTC_MIN = (INT16U)rtc_time->rtc_min;	//modify by xiangyong
	while(R_RTC_BUSY & RTC_MIN_BUSY);
	
	//R_RTC_HOUR = (INT32U)rtc_time->rtc_hour;
	R_RTC_HOUR = (INT16U)rtc_time->rtc_hour;
	while(R_RTC_BUSY & RTC_HR_BUSY);

	//R_RTC_SEC = (INT32U)rtc_time->rtc_sec;
	R_RTC_SEC = (INT16U)rtc_time->rtc_sec;
	/* wait until not busy */
	while(R_RTC_BUSY & RTC_SEC_BUSY);
}
...
void rtc_time_get(t_rtc *rtc_time)	//GP15 vs GP10: same
{
	//vic_irq_disable(VIC_ALM_SCH_HMS);
#if _OPERATING_SYSTEM == 1				// Soft Protect for critical section
	OSSchedLock();		//xiangyong: 这一行将会被执行;
#endif
#if 0
	rtc_time->rtc_sec = gpx_rtc_read(0x0);	//xiangyong:  独立电源RTC;
	rtc_time->rtc_min = gpx_rtc_read(0x1);
	rtc_time->rtc_hour = gpx_rtc_read(0x2);
#else
	rtc_time->rtc_sec = R_RTC_SEC;		//xiangyong: 非独立供电RTC;
	rtc_time->rtc_min = R_RTC_MIN;
	rtc_time->rtc_hour = R_RTC_HOUR;
#endif
#if _OPERATING_SYSTEM == 1	
...
void rtc_day_get(t_rtc *rtc_time)	//GP15 vs GP10: same
{//xiangyong: 此函数对"独立电源RTC"进行读 day 操作;
	//vic_irq_disable(VIC_ALM_SCH_HMS);
#if _OPERATING_SYSTEM == 1				// Soft Protect for critical section
	OSSchedLock();
#endif
...
INT8U gpx_rtc_read(INT8U addr)	//GP15 vs GP10: same
{
	INT8U cmd = 0;
	INT8U data;
...
void gpx_rtc_write(INT8U addr,INT8U data)	//GP15 vs GP10: same
{
	INT8U cmd = 1;
	cmd |= (addr << 3);
...
void rtc_ext_to_int_set(void)	//GP15 vs GP10: same
{ //xiangyong: 此函数将"独立电源RTC"的时分秒读出来, 然后赋给"非独立电源RTC";
	INT16U t;
