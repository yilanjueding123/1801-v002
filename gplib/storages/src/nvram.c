...
INT32U nvnandflash_retrieval(void)
{
   ...
   NVNAND_OS_UNLOCK();
  // Find out Bootcode area at 0
   temp= ND_MAPTABLE[0];
   NVNAND_OS_LOCK();
   Nand_ReadPhyPage(temp*nand_page_nums_per_block_get(),(INT32U)PageBuffer);
   NVNAND_OS_UNLOCK();
  //- Comparing NAND flash bootcode tag
   if (gp_strncmp((INT8S *)PageBuffer, (INT8S *)NAND_FLASH_TAG,10) != 0) {
            gp_free((void *) PageBuffer);
			return 2;
   }
  //- Getting index position
   Resource_area_position = combin_reg_data((INT8U *)&PageBuffer[HEADER_OFFSET_SEC_CNT],2);
#if 0
   Resource_area_position +=temp*nand_page_nums_per_block_get();
#endif
  temp= ND_MAPTABLE[Resource_area_position / (nand_page_nums_per_block_get() * nand_sector_nums_per_page_get()) ];
   temp = temp * nand_page_nums_per_block_get() ; // block addr by page
   temp = temp | ( (Resource_area_position / nand_sector_nums_per_page_get() ) & (nand_page_nums_per_block_get()   -1)) ;
   isector = Resource_area_position % nand_sector_nums_per_page_get() ;//
   NVNAND_OS_LOCK();
   ...
   if (Rspages >1)
   {
     ...
     NVNAND_OS_LOCK();
     for (i =0 ; i<Rspages; i++) 
     {Nand_ReadPhyPage(temp+i,(INT32U)(PageBuffer+256*nand_sector_nums_per_page_get()*i));}
     NVNAND_OS_UNLOCK();
   }
   for (i =0 ; i<total_rs+1; i++) {
     for (j =0 ; j<19; j++)
           RS_TABLE[i].name[j] = *((INT8U *)PageBuffer + j+4 +(23*i)+ (512*isector) );
      RS_TABLE[i].pos = (INT32U) combin_reg_data((INT8U *)PageBuffer +23+ (23*i)+ (512*isector),4);
   }
#endif
   // The least pos is User area position
   User_area_position = RS_TABLE[i-1].pos;
...
INT32U nvnandflash_retrieval(void)
{
  ...
  //- Getting index position is boundary at next block
   ...
   if (Rspages >1)
   {
     ...
     PageBuffer = (INT16U *) gp_malloc_align(Rspages*512*nand_sector_nums_per_page_get(), 16);
     NVNAND_OS_LOCK();
     ret = NandBootReadSector(Resource_area_position,Rspages*nand_sector_nums_per_page_get(),(INT32U)PageBuffer);   
     NVNAND_OS_UNLOCK();
   }
   for (i =0 ; i<total_rs+1; i++) {
     for (j =0 ; j<19; j++)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
#elif  //! NVRAM_MANAGER_FORMAT != NVRAM_V3
