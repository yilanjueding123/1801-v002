...
#define DEC_31TH_2015        2457388

#if MCU_VERSION == GPL32_GP10 
//INT32U LAST_JULIAN_DATE = DEC_31TH_2007;
//+++++++++++++++++++++++++++++++++++++++++++++++
//"独立电源RTC"的Day只有12位, 2^12= 4096 /366= 11.19 年,
//程序中会执行 r_time.rtc_day = day - LAST_JULIAN_DATE;
//所以, 设定的年份必须要大于 LAST_JULIAN_DATE, 即: 2007年12月31日
INT32U LAST_JULIAN_DATE = DEC_31TH_2015; //2015-12-31
//-----------------------------------------------
#else
...
{
	rtc_callback_set(RTC_DAY_INT_INDEX,day_count_callback);	//GP15 vs GP10: same
	rtc_int_set(GPX_RTC_DAY_IEN, RTC_EN_MASK&GPX_RTC_DAY_IEN);	//GP15 vs GP10: same

	
//	rtc_ext_to_int_set();		//xiangyong: 此函数将"独立电源RTC"的时分秒读出来, 然后赋给"非独立电源RTC";
//	day_count = cal_day_get();	//xiangyong: 此函数对"独立电源RTC"进行读 day 操作;
...
void F_TimeData_ext2int(void)
{
	rtc_ext_to_int_set();		//xiangyong: 此函数将"独立电源RTC"的时分秒读出来, 然后赋给"非独立电源RTC";
	day_count = cal_day_get();	//xiangyong: 此函数对"独立电源RTC"进行读 day 操作;
	//++++++++++++++++++++++++++++++++++ xiangyong
	if (day_count == LAST_JULIAN_DATE)
	{
		//day_count += 1005;	//+1+366+365+(365-31-30-31)		//为了让复位后的初始时间是 2010年10月1日
		//day_count += 1217;	//+1+366+365+365+(31+28+31+30)	//为了让复位后的初始时间是 2011年05月01日
		day_count += 122;		//为了让复位后的初始时间是 2016年05月01日
	}
	//----------------------------------
}
...
INT32S cal_time_get(TIME_T  *tm)	//GP15 vs GP10: same
{
    INT32U	s;
    INT32U	j;
    INT32U	gmts;
    ...
    cal_rtc_time_get(&r_time);	//xiangyong: 读取"非独立供电RTC"的"时分秒"并赋给 r_time ;
...
INT32S cal_time_set(TIME_T tm)	//GP15 vs GP10: same
{
    INT32S c;
    INT32U j;
    t_rtc  r_time;
...
{//GP15 vs GP10: same
	...
	rtc_day_get(&r_time);	//xiangyong: 此函数对"独立电源RTC"进行读 day 操作;
...
//	cal_last_jd_store(LAST_JULIAN_DATE);
	...
	r_time.rtc_day = day - LAST_JULIAN_DATE;	//xiangyong: 这一行会被执行;
	rtc_day_set(&r_time);
 #endif
	day_count = day;
}
...
static void cal_rtc_time_get(t_rtc *rtc_time)	//GP15 vs GP10: same
{
	rtc_time_get(rtc_time);	//xiangyong: 这一行将会被执行;
}
/*
#if MCU_VERSION == GPL32_GP10
static void cal_last_jd_store(INT32U last_jd)
